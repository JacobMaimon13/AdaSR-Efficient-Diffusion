...
class ResnetBlock(nn.Module):
    ...
    def forward(self, x, t_embed, extra_feats):
        ...
        if extra_feats is not None:
            ...
            extra_feats = FV.resize(extra_feats, [x.shape[2], x.shape[3]],
    ...
    def __init__(self, num_channels, time_channels, extra_channels, out_channels=None):
        ...
        for i in range(len(channels)):
            for j in range(num_blocks[i]):
                self.downs.append(ResnetBlock(prev_channel, channels[i], time_channels, extra_channels))
        ...
        for i in range(len(channels) - 1, -1, -1):
            for j in range(num_blocks[i] + 1):
                ...
                self.ups.append(ResnetBlock(prev_channel + res_channel, channels[i], time_channels, extra_channels))
